...
def openai_request_mock(openaikey: str, endpoint: str, parameters: Dict[str, Any]):
    # actually we don't care in tests what response it returns because we test data caching only
...
class CacheTests(TestCase):
    ...
    def exec(self, request_data, endpoint):
        ...
        cache_hit_true_counts = Log.objects.filter(
            **filter_kwargs | {"cache_hit": True}
        ).count()
        cache_hit_false_counts = Log.objects.filter(
            **filter_kwargs | {"cache_hit": False}
        ).count()
        self.assertEqual(cache_hit_true_counts, 0)
        self.assertEqual(cache_hit_false_counts, 1)
        ...
        self.assertEqual(cache_hit_true_counts, 1)
        self.assertEqual(cache_hit_false_counts, 1)
